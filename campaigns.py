...
* **403 → не долбим ОДНИМ пушем, но юзера не хороним.** Попытка помечается в
  notifications с РЕАЛЬНЫМ `kind` кампании (+ `meta.blocked`): тот же пуш не
  повторяется (`was_notified`), и «касание» занимает слот — маркетинг под месячный
  кап (≤1 попытка/30д даже заблокировавшему), транзакционные не под капом →
  пытаются по сроку. Из выборки заблокировавших не исключаем — только по opt-out.
    ...
    for u in users:
        ...
        for _ in range(MAX_RETRIES):
            ...
            if status == "sent":
                ...
                # Помечаем ПОПЫТКУ с реальным kind кампании: это «касание», и оно
                # обязано занять слот. Для маркетинга — месячный кап (marketing_sent_since
                # считает kind='marketing'): заблокировавший получит ≤1 маркетинговую
                # попытку в 30 дней, а не по одной на каждую кампанию за проход.
                # Транзакционные под кап не попадают → продолжают пытаться по сроку.
                # was_notified (тот же пуш не повторяем) работает независимо от kind.
                db.log_notification(
                    tg_id, campaign.name, campaign.kind,
                    {**(campaign.meta or {}), "blocked": True},
                )
