...
def sharing_whitelist_add(username: str, added_by: int) -> None:
    """Пометить юзера «это нормально» — детектор перестаёт его алертить.

    Нужно, потому что сигнал шумный: у кого-то честно офис + дом + телефон дают
    устойчивый мультилокейшн. Кнопкой из алерта Артём отсеивает такие разы.
    """
    with _conn() as c:
        c.execute("""
            CREATE TABLE IF NOT EXISTS sharing_whitelist (
        ...
        """)
        c.execute(
            "INSERT INTO sharing_whitelist (username, added_by, added_at) VALUES (?, ?, ?) "
            "ON CONFLICT(username) DO UPDATE SET added_by=excluded.added_by, added_at=excluded.added_at",
            (username, added_by, _now_iso()),
        )


def sharing_whitelisted() -> set[str]:
    with _conn() as c:
        try:
            return {r[0] for r in c.execute("SELECT username FROM sharing_whitelist")}
        except sqlite3.OperationalError:
            return set()  # таблицы ещё нет — никто не в вайтлисте
