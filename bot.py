...
async def _send_expire_reminder(tg_id: int, hours_left: float, label: str) -> None:
    ...
    if label == "expired":
        text = (
            ...
            "VPN перешёл в captive-режим — открыты только наш сайт и "
            "страница оплаты, остальное заблокировано. Продли подписку "
            "одним кликом — та же ссылка в клиенте сразу станет рабочей."
        )
    elif label == "2h":
